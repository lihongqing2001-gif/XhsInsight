  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'local') {
      if (!apiKey.startsWith('AIza')) {
         setError('无效的 Gemini API Key 格式');
         return;
      }
      onLocalMode(apiKey);
      return;
            ...
            {mode === 'local' ? (
               <div className="bg-blue-50 p-3 rounded-lg text-xs text-blue-700 mb-2">
                 ...
                 <ul className="list-disc list-inside">
                   <li>无需注册账号，所有数据存储在本地浏览器。</li>
                   <li>需手动提供 <strong>Gemini API Key</strong>。</li>
                   <li>登录后请在“Cookie资产”中添加小红书Cookie。</li>
                 </ul>
               </div>
            ) : null}
